...
#[test]
fn generated_operation_free_signal_compiles_without_datom_codec() {
    let directory = format!(
        "{}/empty-signal-without-datom-{}",
        env!("CARGO_TARGET_TMPDIR"),
        std::process::id()
    );
    let _ = std::fs::remove_dir_all(&directory);
    ...
    std::fs::write(
        format!("{directory}/Cargo.toml"),
        "[package]\nname = \"empty-signal-without-datom\"\nversion = \"0.1.0\"\nedition = \"2024\"\n[features]\ndatom = []\n[dependencies]\nrkyv = { version = \"0.8\", default-features = false, features = [\"std\", \"bytecheck\", \"little_endian\", \"pointer_width_32\", \"unaligned\"] }\n",
    ).expect("temporary manifest");
    std::fs::write(
        format!("{directory}/src/lib.rs"),
        include_str!("generated/empty-signal.rs"),
    )
    .expect("generated Signal source");
    assert!(
        Command::new("cargo")
