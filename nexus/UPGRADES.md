...
```sh
nix run github:LiGoldragon/ethos-zero#ethos-zero-migrate-v1 -- \
  "$XDG_STATE_HOME/ethos-zero-nexus/ethos-zero-nexus.sema" \
  "$XDG_STATE_HOME/ethos-zero-nexus/ethos-zero-nexus-v2.sema"
```

The converter reads the old application configuration and assembly rows plus
its retired Datomic source map, then
...
the emitted v2 store. The converter transfers application rows only; it does
not copy sema transaction or engine-log history. Retain the v1 store for the
old version's inspection route and its original engine history until neither
is needed; do not run the new serving Nexus against it.
